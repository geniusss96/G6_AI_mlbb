...
absence of tactical decisions, determinism, and architecture isolation.
"""
...
class TestTacticalStateBuilder(unittest.TestCase):
    ...
    def test_player_hp_and_position_extraction(self):
        ws = WorldState(
            ...
            player=PlayerState(
                ...
                hp=HPObservation(value=0.45, confidence=0.9, visible=True),
                is_visible=True,
            ),
        )
        ts = self.builder.build(ws)
        self.assertEqual(ts.timestamp, 12.5)
        self.assertEqual(ts.player_position, Vector2(500.0, 400.0))
        self.assertAlmostEqual(ts.player_hp_ratio, 0.45)
    ...
    def test_dead_player_flag(self):
        ws = WorldState(
            ...
            player=PlayerState(
                position=Vector2(500.0, 400.0),
                hp=HPObservation(value=0.0, confidence=1.0, visible=False),
                is_visible=False,
            ),
        )
        ts = self.builder.build(ws)
    ...
    def test_optional_player_fallback(self):
        ...
        ts = self.builder.build(ws)
        self.assertEqual(ts.player_position, Vector2(772.0, 360.0))
        self.assertEqual(ts.player_hp_ratio, 1.0)
        self.assertFalse(ts.is_player_dead)

    def test_nearest_enemy_and_enemy_count(self):
        p = Vector2(700.0, 300.0)
        e1 = EnemyState(position=Vector2(900.0, 300.0), bbox=(880, 280, 920, 320), confidence=0.9)  # dist 200
        e2 = EnemyState(position=Vector2(1100.0, 300.0), bbox=(1080, 280, 1120, 320), confidence=0.8) # dist 400

        ws = WorldState(
            timestamp=25.0,
            player=PlayerState(position=p, hp=HPObservation(value=1.0, confidence=1.0, visible=True)),
            enemies=[e1, e2],
        )
        ts = self.builder.build(ws)
        self.assertTrue(ts.enemy_visible)
        self.assertEqual(ts.enemy_count, 2)
        self.assertAlmostEqual(ts.nearest_enemy_distance, 200.0)
        self.assertEqual(ts.nearest_enemy_position, Vector2(900.0, 300.0))

    def test_empty_enemy_list(self):
        ws = WorldState(timestamp=30.0, enemies=[])
        ts = self.builder.build(ws)
    ...
    def test_nearest_minion_and_minion_count(self):
        p = Vector2(700.0, 300.0)
        m_enemy = MinionState(position=Vector2(850.0, 300.0), bbox=(830, 280, 870, 320), confidence=0.8, is_enemy=True)  # dist 150
        m_ally = MinionState(position=Vector2(600.0, 300.0), bbox=(580, 280, 620, 320), confidence=0.8, is_enemy=False)
        ...
        ts = self.builder.build(ws)
        self.assertEqual(ts.minion_count, 1)  # only enemy minions
        self.assertAlmostEqual(ts.nearest_minion_distance, 150.0)
        self.assertEqual(ts.nearest_minion_position, Vector2(850.0, 300.0))

    def test_empty_minion_list(self):
        ws = WorldState(timestamp=40.0, minions=[])
        ts = self.builder.build(ws)
    ...
    def test_skill_readiness_and_combo_flag(self):
        ...
        self.assertFalse(ts.can_combo)  # s2 is False -> False

        ws2 = WorldState(
            timestamp=46.0,
            skills=SkillState(s1_ready=False, s2_ready=True, ultimate_ready=True),
        )
    ...
    def test_turret_proximity(self):
        p = Vector2(700.0, 300.0)
        t_danger = TurretState(position=Vector2(1000.0, 300.0), bbox=(980, 280, 1020, 320), confidence=0.9, is_enemy=True)  # dist 300 < 420
        ...
        t_safe = TurretState(position=Vector2(1200.0, 300.0), bbox=(1180, 280, 1220, 320), confidence=0.9, is_enemy=True)  # dist 500 > 420
        ws_safe = WorldState(
            timestamp=51.0,
            player=PlayerState(position=p, hp=HPObservation(value=1.0, confidence=1.0, visible=True)),
            turrets=[t_safe],
    ...
    def test_deterministic_conversion(self):
        ws = WorldState(
            timestamp=70.0,
            enemies=[EnemyState(position=Vector2(900.0, 300.0), bbox=(880, 280, 920, 320), confidence=0.9)],
        )
    ...
    def test_architecture_isolation(self):
        """brain.state must not import hardware, actions.executor, OpenCV, or YOLO."""
        ...
        for f in forbidden:
            self.assertFalse(hasattr(state_module, f))
