...
class HPObservation:
    ...
    confidence: float   # 1.0 when clearly visible, 0.0 when missing/unavailable
    visible: bool       # True if the HP bar is detected
...
class HPDetector:
    ...
    def detect_player_hp(
        self,
        frame: np.ndarray,
        self_target: Optional[
            Union[Tuple[float, float], List[float], Detection]
        ] = None,
    ) -> HPObservation:
        """
        ...
        Returns:
            HPObservation with normalized value in [0.0, 1.0].
            When the HP bar is unavailable, visible=False and confidence=0.0.
        """
        ...
        # IMPORTANT:
        # Missing HP bar is NOT the same thing as 10% HP.
        if not visible:
            return HPObservation(
                value=1.0,
                confidence=0.0,
    ...
    def is_real_enemy_hp_bar(
        ...
        """
        Enemy HP-bar color verification.

        Enemy heroes are expected to have red HP bars.
        """
    ...
    def detect_red_hp_bars(self, frame: np.ndarray) -> List[List[float]]:
        ...
        Returns:
            List of [cx, cy, bw, bh] bounding boxes in reference
            resolution (1544x720).
        """
    ...
    def is_real_enemy_minion(
        ...
        green_ratio = np.count_nonzero(green_mask) / float(
